 ...
 * ------------------------------------------- */
$query2 = $pdo->prepare('SELECT id_alternatif, nama_aplikasi FROM alternatif');
$query2->execute();			
...
foreach($alternatifs as $alternatif):
	...
	$ranks[$alternatif['id_alternatif']]['nama_aplikasi'] = $alternatif['nama_aplikasi'];
					...
					<div class="table-responsive">
						<table class="table table-striped text-center table-hover table-bordered">
							<thead class="table-primary">
								<tr class="super-top">
									<th rowspan="2" class="super-top-left">Nama Aplikasi</th>
							...
							<tbody>
								<?php foreach($alternatifs as $alternatif): ?>
									<tr>
										<td><?php echo $alternatif['nama_aplikasi']; ?></td>
										<?php						
										foreach($kriterias as $kriteria):
											$id_alternatif = $alternatif['id_alternatif'];
											$id_kriteria = $kriteria['id_kriteria'];
						...
						<table class="table table-striped text-center table-hover table-bordered" id="table_id">
							<thead class="table-primary">					
								<tr>
									...
									<th class="super-top-left">Nama Aplikasi</th>
							...
							<tbody>
								<?php foreach($sorted_ranks as $alternatif ): ?>
									<tr>
										...
										}elseif (($ket>0.20 ||$ket>0.2)  && ($ket <=0.40 || $ket<=0.4)){ echo "Tidak Direkomendasikan";}
										elseif (($ket>0.40 ||$ket>0.4)  && ($ket <=0.60 || $ket<=0.6)){ echo "Cukup Direkomendasikan";}
										elseif (($ket>0.60 ||$ket>0.6)  && ($ket <=0.80 || $ket<=0.8)){ echo "Lumayan Direkomendasikan";}
										else echo "Sangat Direkomendasikan"
